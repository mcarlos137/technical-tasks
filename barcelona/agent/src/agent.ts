...
// Alias that tracks Google's current Flash model, so the demo keeps working as versions are retired.
...
export async function runAgentTurn(
  ...
  for (let round = 0; round <= MAX_TOOL_ROUNDS; round++) {
    ...
    if (calls.length === 0) {
      return { reply: response.text?.trim() || "Sorry, I couldn't produce an answer.", toolCalls };
    }
    const parts = [];
